...
import dataset
import utils
from segmodeltest import model, ENCODER_WEIGHTS
######## Settings ########
...
segNet = model
print(segNet) # print full net
model_parameters = filter(lambda p: p.requires_grad, segNet.parameters())
params = sum([np.prod(p.size()) for p in model_parameters])
print("Initialized segNet with {} trainable params ".format(params))

#-----------------------------------------------------------------------------------------------------------------
#segNet.apply(ENCODER_WEIGHTS)
#if len(doLoad)>0:
    #segNet.load_state_dict(torch.load(doLoad))
...
optimizerG = optim.Adam(segNet.parameters(), lr=lrG, betas=(0.5, 0.999), weight_decay=0.0)
...
torch.save(segNet.state_dict(), prefix + "modelSEG" )
